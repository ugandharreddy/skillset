...
//            		technology.concat(", Proficiency:",item.tProf);
            	...
            	var pathrrr="/ProficiencyManagementClient";//${pageContext.servletContext.contextPath};
            	...
            	document.getElementById("mybtn"+i).addEventListener("click", function(){
            	   ...
            	   var empMailId=item.empMailId;
					...
					location.href="/SkillProficiencyClient/employeeDetailsById/"+empId+"/"+name+"/"+empMailId;
...
$('#rmgskill').on('keyup', function (temp) {
          ...
          {    
			...
			$('.es-list:first').empty();
        	 $.each(message, function(i, item) {
	  		   	...
	  		   	$('.es-list:first').append("<li>"+message[i]['skillName'].trim()+"</li>");
	  		  $('.es-list:first').show();
          ...
          {    
			...
			$('.es-list').eq(1).empty();
        	 ...
        	 $.each(message, function(i, item) {
	  		   	...
	  		   	$('.es-list').eq(1).append("<li>"+message[i]['projectName']+"</li>");
	  		 // $("your classs").eq(1);
	  		  $('.es-list').eq(1).show();
...
$('#freeSearch').on('keypress', function (temp) {
	...
	if ((temp.which == '13')&&(this.value.trim())) {
...
//Create Base64 Object
 ...
 var encodedString = Base64.encode(freeSearch);
			var path1=document.getElementById("contextPaht").value;
			  ...
			  $.ajax({
		          type: "POST",
		          url:path1+"/searchByKeyword/"+encodedString,
		          success: function(message)
		          { 
